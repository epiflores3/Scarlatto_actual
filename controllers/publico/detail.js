...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (JSON.status) {
        ...
        document.getElementById('imagen').src = SERVER_URL.concat('imagenes/productos/', JSON.dataset.imagen_principal);
        ...
        document.getElementById('material').textContent = JSON.dataset.nombre_material;
        document.getElementById('marca').textContent = JSON.dataset.marca;
        document.getElementById('id_producto').value = JSON.dataset.id_producto;
        fillSelectFilter(PRODUCTO_API, 'readTallasProducto', 'talla', JSON.dataset.id_producto );
        // fillSelectFilter(PRODUCTO_API, 'readMaterialesProducto', 'material',JSON.dataset.id_producto);
    } else {
...
SHOPPING_FORM.addEventListener('submit', async (event) => {
    ...
    if (JSON.status) {
        sweetAlert(1, JSON.message, true, 'mi_cesta.html');
...
async function funcionpreciotallaautomatico(){
const FORM = new FormData();
FORM.append('id_producto',document.getElementById('id_producto').value );
FORM.append('id_talla',document.getElementById('talla').value );
const JSON = await dataFetch(PRODUCTO_API, 'readPrecioProducto', FORM);
    // Se comprueba si la respuesta es satisfactoria, de lo contrario se muestra un mensaje con la excepción.
    if (JSON.status) {
        // Se colocan los datos en la página web de acuerdo con el producto seleccionado previamente.
